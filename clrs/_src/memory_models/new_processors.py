...
class Gatv2_NTM(GATv2):
    ...
    def __call__(
        ...
        adj_mat_new, hidden_new, new_edge_fts, node_fts_new = extend_features(adj_mat, edge_fts, hidden, node_fts,self.read_nodes_amount,self.write_nodes_amount,self.write_node_fts_layer,self.write_edge_fts_layer,self.read_node_fts,self.read_edge_fts_layer)
        # assert adj_mat.shape == (b, n, n)

        ################################################
        # Network is called here
        ret = super().__call__(node_fts_new, new_edge_fts, graph_fts, adj_mat_new, hidden_new, **unused_kwargs)
        ################################################

        memory_input, ret = self.memory_type.prepare_memory_input(ret, n)

        self.read_node_fts, self.lstm_state = self.memory_type(memory_input,
                                                                  self.lstm_state)
