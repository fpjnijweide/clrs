...
class DNCAccessModule(hk.RNNCore):
    ...
    *   `addressing.FreenessAllocator` for keeping track of memory usage, where
    ...
    def __init__(self,
        ...
        self.memory_layer = hk.initializers.RandomNormal()
        self.read_weights_layer = hk.initializers.RandomNormal()
        self.write_weights_layer = hk.initializers.RandomNormal()
        self.linkage_layer = hk.initializers.RandomNormal()
        self.usage_layer = hk.initializers.RandomNormal()


    def initial_state(self, batch_size):
        return AccessState(memory=jnp.zeros(self.state_size.memory),
                           read_weights=self.read_weights_layer(self.state_size.read_weights),
                           write_weights=self.write_weights_layer(self.state_size.write_weights),
                           linkage=self.linkage_layer(self.state_size.linkage),
                           usage=self.usage_layer(self.state_size.usage))
