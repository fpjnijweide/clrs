...
def _vector_norms(m):
    squared_norms = jnp.reduce_sum(m * m, axis=2, keepdims=True)
...
def weighted_softmax(activations, strengths, strengths_op):
    ...
    softmax = hk.BatchApply(module_or_op=nn.softmax)
...
class TemporalLinkage(hk.RNNCore):
    ...
    def _precedence_weights(self, prev_precedence_weights, write_weights):
        ...
        """
        with jnp.name_scope('precedence_weights'):
            write_sum = jnp.reduce_sum(write_weights, 2, keepdims=True)
...
class Freeness(hk.RNNCore):
    ...
    *   Decreased by reading, where usage is decreased after reading from a
    ...
    def _build(self, write_weights, free_gate, read_weights, prev_usage):
        ...
        write_weights = jnp.stop_gradient(write_weights)
    ...
    def _allocation(self, usage):
        ...
        nonusage = 1 - usage
        sorted_nonusage, indices = nn.top_k(
