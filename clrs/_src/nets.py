...
from clrs._src.memory_models.storage_nodes import StorageNodes
    ...
    def __call__(self, features_list: List[_Features], repred: bool,
        ...
        if self.use_memory and self.use_memory != "":
            if self.use_memory == "LSTM":
                ...
                self.write_edge_fts_layer = hk.initializers.RandomNormal()
                self.read_edge_fts_layer = hk.initializers.RandomNormal()
            elif self.use_memory == "DNC":
                self.memory = DNCAccessModule(name='processor_DNC',memory_size=self.memory_size,word_size=self.hidden_dim)
                self.read_node_fts = None
                self.write_node_fts_layer = hk.initializers.RandomNormal(stddev=0.5)
                self.read_node_fts_layer = hk.initializers.RandomNormal(stddev=0.5)
                self.write_edge_fts_layer = hk.initializers.RandomNormal()
...
class NetChunked(Net):
    ...
    def _msg_passing_step(self,
        ...
        if self.use_memory == "LSTM":
            ...
            # TODO other memories. net chunked here
            memory_state = None
